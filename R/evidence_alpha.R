...
#'
#'
evidence_alpha <- function(x1, x2, n1, n2, alpha0, alpha1, alpha2, i, j, N2, n_bootstrap, seed = NULL) {
  ...
  GA_ev <- ga(type = "real-valued",
              fitness = function(x) {
                densBB(x1, x2, n1, n2, u, l, alpha0, alpha1, alpha2, i, equal_thetas = TRUE)(x[1])
              },
  ...
  densBB <- densBB(x1, x2, n1, n2, u, l, alpha0, alpha1, alpha2, i, equal_thetas = TRUE)

  ev10 <- mean(apply(Thetas, 1, function(t) {
    I(densBB_ev(t[1], t[2]) < densBB_ev(supremo_ga_ev))
  }), na.rm = TRUE)
